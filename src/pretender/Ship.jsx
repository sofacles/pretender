...
const Ship = ({ isThrusting, x, y, reversed, showMarkers }) => {
  ...
  useEffect(() => {
    ...
    const killInterval = setInterval(() => {
      ...
      setRandomX_top1(Math.floor(TOP_LEFT_X + Math.random() * 99));
      setrandomY_top1(TOP_LEFT_Y + Math.floor(Math.random() * 4));

      setcolorIndex_top2(Math.floor(Math.random() * 6));
      setRandomX_top2(Math.floor(TOP_LEFT_X + Math.random() * 101));
      setrandomY_top2(TOP_LEFT_Y + Math.floor(Math.random() * 4));

      setcolorIndex_bottom1(Math.floor(Math.random() * 6));
      setRandomX_bottom1(Math.floor(TOP_LEFT_X + Math.random() * 99));
      setrandomY_bottom1(TOP_LEFT_Y + Math.floor(Math.random() * 4));
      
      setcolorIndex_bottom2(Math.floor(Math.random() * 6));
      setRandomX_bottom2(Math.floor(TOP_LEFT_X + Math.random() * 101));
      setrandomY_bottom2(TOP_LEFT_Y + Math.floor(Math.random() * 4));
    }, 100);
    return () => {clearInterval(killInterval)}
  }, []);
  return (
    <g transform={`translate(${x},${y}) scale(0.25)`}>
      {showMarkers && <text x="0" y="0" className="horizontal-rect">top</text>}
      <g transform={`${reversed ? "scale(-1, 1)" : ""}`}>
      ...
      <path d="M36,58l3,-3 9,-1 1,-4 3,-4 -5,-7 -5,-7 0,1 -6,-1 -1,-4 -5,-8 -16,1 -3,7 -8,1 -2,6 -12,14 -39,-1 -1,11 -43,1 -9,3 1,9 9,4 17,0 1,0 8,1 13,0 22,-2 7,10 16,5 16,-6 14,-1 z" fill="#acafa5" />
      <path d="M0,64l20,2 10,-11 7,-2 9,5 18,3 -8,9 -13,2 -11,11 -16,1 0,-10 z" fill="#995997" />
     
    </g>
    {showMarkers && <text x="200" y="200" className="my-text">{`y: ${y}`}</text>}
    </g>
  );
};
